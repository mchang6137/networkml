...
class PS (Entity):
    ...
    def lastbitrecv(self, packet):
        ...
        if not packet.MF and self.name in self.ctx.ps_num_items:
            ...
            #     self.bytes_hit = 1
            if self.ctx.multi_step and min([val for key, val in list(self.bits_received.items())]) > 0 and self.bits_sent == 0 and self.phase == 1:
                self.ctx.start_time = self.ctx.now
            if self.ctx.multi_step and min([val for key, val in list(self.bits_received.items())]) > 0 and self.bits_sent == 0 \
            ...
            if self.received_packets >= self.ctx.ps_num_items[self.name]:
                if self.ctx.verbosity:
                    ...
                    print("{} bits received".format(min([val for key, val in list(self.bits_received.items())])))
    ...
    def redistribute(self):
        ...
        if not self.ctx.in_network_computation and len(self.bits_received) != len(self.ctx.workers):
            return
        bit_recv = min([val for key, val in list(self.bits_received.items())])
